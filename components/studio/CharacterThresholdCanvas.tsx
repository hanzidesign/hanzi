...
} from '@/components/studio/threshold-material'
import {
...
export default function CharacterThresholdCanvas() {
  ...
  return (
    <div
      data-testid="character-threshold-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterThresholdScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Threshold...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterThresholdScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  useEffect(() => {
    const nextSource = geometryResult ? createThresholdSourceScene(geometryResult) : null
  ...
  const material = useMemo(() => createThresholdShaderMaterial({
    controls: {},
...
type ThresholdSourceScene = {
  scene: Scene
  group: Group
  dispose: () => void
}

function createThresholdSourceScene(
  geometryResult: CharacterMeshGeometryResult,
): ThresholdSourceScene {
