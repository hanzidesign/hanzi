...
} from '@/components/studio/blockify-material'
import {
...
export default function CharacterBlockifyCanvas() {
  ...
  return (
    <div
      data-testid="character-blockify-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterBlockifyScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Blockify...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterBlockifyScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  const controls = useStudioStore((store) => store.grainradEffect.controls.blockify)
  ...
  const sourceRef = useRef<BlockifySourceScene | null>(null)
