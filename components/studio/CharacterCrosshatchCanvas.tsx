...
} from '@/components/studio/crosshatch-material'
import {
...
export default function CharacterCrosshatchCanvas() {
  ...
  return (
    <div
      data-testid="character-crosshatch-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterCrosshatchScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Crosshatch...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterCrosshatchScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  useEffect(() => {
    const nextSource = geometryResult ? createCrosshatchSourceScene(geometryResult) : null
  ...
  const material = useMemo(() => createCrosshatchShaderMaterial({
    controls: {},
...
type CrosshatchSourceScene = {
  scene: Scene
  group: Group
  dispose: () => void
}

function createCrosshatchSourceScene(
  geometryResult: CharacterMeshGeometryResult,
): CrosshatchSourceScene {
