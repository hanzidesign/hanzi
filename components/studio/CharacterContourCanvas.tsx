...
export default function CharacterContourCanvas() {
  ...
  return (
    <div
      data-testid="character-contour-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterContourScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Contour...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterContourScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  const sourceRef = useRef<ContourSourceScene | null>(null)
