...
function brightnessMapControl(groups = STUDIO_COMMON_PROCESSING_GROUPS) {
  return groups
...
describe('Brightness Map defaults', () => {
  it('uses the neutral default and the Matrix Rain-specific maximum contract', () => {
    ...
    expect(brightnessMapControl(getStudioProcessingGroups('matrix-rain'))).toMatchObject({
      defaultValue: 1,
      min: 0,
      max: 6,
    })
  ...
  it('uses the neutral Matrix Rain default in initial controls, Reset, and compiler fallback', () => {
    ...
    store.getState().setStudioEffectControl('matrix-rain', 'brightness-map', 6)
