...
} from '@/components/studio/vhs-material'
import {
...
export default function CharacterVhsCanvas() {
  ...
  return (
    <div
      data-testid="character-vhs-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterVhsScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for VHS...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterVhsScene({
  ...
  const controls = useStudioStore((store) => store.studioEffect.controls['vhs'])
  ...
  useEffect(() => {
    const nextSource = geometryResult
      ? createVhsSourceScene(geometryResult, meshSettings.repeat)
      : null
  ...
  useEffect(() => {
    ...
    if (source) {
      applyVhsSourceBackground(source, activeBackground)
      markSourceRenderDirty(sourceRenderStateRef.current)
    }
