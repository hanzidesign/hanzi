...
describe('Dithering shader material', () => {
  it('samples the selected Character texture on a source-pixel Bayer 8x8 grid', () => {
    ...
    expect(DITHERING_FRAGMENT_SHADER).toContain('sourcePixel')
  ...
  it('maps shared Processing and Post-Processing controls into the Dithering pipeline', () => {
    ...
    expect(material.uniforms.u_grainIntensity.value).toBe(35)
    expect(material.uniforms.u_grainSize.value).toBe(2)
    expect(material.uniforms.u_grainSpeed.value).toBe(50)
