...
export default function CharacterHalftoneCanvas() {
  ...
  return (
    <div
      data-testid="character-halftone-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterHalftoneScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Halftone...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterHalftoneScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  const controls = useStudioStore((store) => store.studioEffect.controls.halftone)
  ...
  useEffect(() => {
applyHalftoneUniforms(material, withoutSharedControllerValues(controls))
  }, [controls, material])
