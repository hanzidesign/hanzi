...
describe('Grainrad effect renderer routing contract', () => {
  ...
  it('routes each implemented effect to its own renderer and never falls back to ASCII', async () => {
    ...
    expect(pixelSortCanvasSource).toContain('createPixelSortTrailResources')
    expect(pixelSortCanvasSource).toContain('renderPixelSortTrails')
    expect(pixelSortCanvasSource).not.toContain('PixelSortWorkerClient')
    expect(pixelSortCanvasSource).not.toContain('readRenderTargetPixelsAsync')
    expect(pixelSortCanvasSource).not.toContain('DataTexture')
    expect(pixelSortCanvasSource).toContain('pendingExportAckRef')
    expect(pixelSortCanvasSource).toContain('preparedExportAckRef')
