...
} from '@/components/studio/wave-lines-material'
import {
...
export default function CharacterWaveLinesCanvas() {
  ...
  return (
    <div
      data-testid="character-wave-lines-canvas"
      data-state={state}
      style={{
        position: 'relative',
        width: '100%',
    ...
    >
      ...
      >
        ...
        <CharacterWaveLinesScene svgData={svgData} svgLoadError={svgLoadError} />
      </Canvas>
      {state !== 'ready' ? (
        <div
          role="status"
        ...
        >
          {svgLoadError ?? 'Loading selected Character for Wave Lines...'}
        </div>
      ) : null}
    </div>
  )
...
function CharacterWaveLinesScene({
  svgData,
  svgLoadError,
}: {
  svgData: string
  ...
  useEffect(() => {
    const nextSource = geometryResult ? createWaveLinesSourceScene(geometryResult) : null
  ...
  const material = useMemo(() => createWaveLinesShaderMaterial({
    controls: {},
  ...
  useEffect(() => {
applyWaveLinesUniforms(material, withoutSharedControllerValues(controls))
  }, [controls, material])
...
type WaveLinesSourceScene = {
  scene: Scene
  group: Group
  dispose: () => void
}

function createWaveLinesSourceScene(
  geometryResult: CharacterMeshGeometryResult,
): WaveLinesSourceScene {
