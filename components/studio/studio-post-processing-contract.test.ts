...
describe('Studio shared post-processing compositor contract', () => {
  it('mounts one real compositor in the shared render canvas', async () => {
    ...
    expect(postProcessing).toContain("readNumber(controls?.['scanline-opacity'], 0.2)")
    ...
    expect(postProcessing).toContain("clamp(readNumber(controls?.['crt-amount'], 0.1), 0, 0.5)")
